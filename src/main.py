...
from flask import Flask, redirect, g, request, flash
import sqlite3
...
class Note(BaseModel):
    id: UUID4 = uuid.uuid4()
    note: str = Field(max_length=200) # constr(strip_whitespace=True, max_length=200)  
...
def new_note_page():
    temp = env.get_template("new_note.html")
    return temp.render()
...
def add_note():
    ...
    db = get_db()
    try:
        db.execute(insert, (str(note.id), note.note, note.creation_date, note.deadline, note.alert_level, note.status))
        db.commit()
    except:
...
def get_all() -> list[Note]:
    ...
    for row in db.execute("SELECT * FROM notes"):
        print(row)
        notes.append(Note(id=row[0], note=row[1], creation_date=row[2], deadline=row[3], alert_level=row[4], status=row[5]))
    return notes 

# async def get_note(id: int):
#     cursor.execute("SELECT * FROM notes WHERE id=(?)", (id,))
#     data = cursor.fetchone()
#     note = Note(id=data[0], note=data[1], creation_date=data[2], deadline=data[3], alert_level=data[4])
#     return note
...
#     cursor.execute("UPDATE notes SET note = (?), deadline = (?), alert_level = (?) WHERE id = (?)",
#                    (note.note, note.deadline, note.alert_level, id))
#     return note
#
